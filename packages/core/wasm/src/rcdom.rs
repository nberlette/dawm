...
macro_rules! element_node {
  ($name:expr, $attrs:expr) => {
    element_node!($name, $attrs, None)
  };
  ($name:expr, $attrs:expr, $template_contents:expr) => {
    element_node!($name, $attrs, $template_contents, false)
  };
  ($name:expr, $attrs:expr, $template_contents:expr, $mathml_annotation_xml_integration_point:expr) => {
