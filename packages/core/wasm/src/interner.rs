...
impl Interner {
  ...
  pub fn extend<T: AsRef<[S]>, S: ToString>(mut self, strings: T) -> Self {
    ...
    let mut i = 0;
    while i < strings.len() {
      self.intern(strings[i].to_string());
      i += 1;
    }
    self
  }
  ...
  pub fn intern<S: AsRef<str>>(&mut self, s: S) -> u32 {
    if self.index.is_none() && !self.table.is_empty() {
      self.rebuild_index();
    }

    let index = self.index.get_or_insert_with(HashMap::new);
    let s_ref = s.as_ref();
    if let Some(&idx) = index.get(s_ref) {
      idx
    } else {
      let idx = self.table.len() as u32;
      let owned = s_ref.to_string();
      self.table.push(owned.clone().into());
  ...
  fn rebuild_index(&mut self) {
    let index = self.index.get_or_insert_with(HashMap::new);
    index.clear();
    for (idx, value) in self.table.iter().enumerate() {
      index.insert(value.to_string(), idx as u32);
    }
  }
}
