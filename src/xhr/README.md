...
The [dawm] XML submodule `xhr` provides a runtime-agnostic implementation of
several standard [`XMLHttpRequest`] APIs, allowing asynchronous HTTP requests to
be made within the context of a _headless DOM environment_[^1].
...
```ts
import { XMLHttpRequest } from "dawm/xml/xhr";
...
Unlike the original `xhr` ponyfill, dawm's `XMLHttpRequest` implementation has
been enhanced and integrated with the [`XMLDocument`] implementation from the
parent `dawm/xml` module.

This allows the `"document"` response type to be used in the same way as the
standard DOM implementation, for a one-step fetch/parse workflow of HTML/XML
...
```ts
import { XMLHttpRequestEventTarget } from "dawm/xml/xhr";
...
```ts
import { XMLHttpRequestUpload } from "dawm/xml/xhr";
